...
public class PixelGunGameManager : MonoBehaviourPunCallbacks
{
    ...
    void Start()
    {
        ...
        if(playerPrefab == null) Debug.LogError("playerPrefab is null / PixelGunGameManager/ GameManagerGO");
        if (PhotonNetwork.IsConnected && playerPrefab != null) PhotonNetwork.Instantiate(playerPrefab.name,randomPoint,Quaternion.identity);
